...
def scanBlocks(chain,start_block,end_block,contract_address):
    ...
    if end_block - start_block < 30:
        ...
        for evt in events:
            ...
            token_ls.append(evt.args['token'])
            recipient_ls.append(evt.args['recipient'])
            amount_ls.append(evt.args['amount'])
            transactionHash_ls.append(evt.transactionHash.hex())
            address_ls.append(evt.address)
    else:
        for block_num in range(start_block,end_block+1):
            ...
            for evt in events:
                ...
                transactionHash_ls.append(evt.transactionHash.hex())
                address_ls.append(evt.address)
    log_pd = pd.DataFrame({'chain':chain_ls,
                            'token':token_ls,
                           'recipient':recipient_ls,
                           'amount':amount_ls,
                           'transactionHash':transactionHash_ls,
                           'address':address_ls})
    log_pd.to_csv('deposit_logs.csv')
