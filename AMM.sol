...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		sellAmountWithFee = sellAmount * (10000 - feebps) / 10000;
		inReserve         = ERC20(inToken).balanceOf(address(this)); 
		outReserve        = ERC20(outToken).balanceOf(address(this));

		swapAmt           = outReserve * sellAmountWithFee / (inReserve + sellAmountWithFee);
		ERC20(inToken).transferFrom(msg.sender, address(this), sellAmount);
		ERC20(outToken).transfer(msg.sender,swapAmt);
		require(ERC20(outToken).balanceOf(address(this))>0,'Cannot drain the AMM');
